...
static int printKernelBuffer(char *buf, int count) {
    ...
    while (buffer < (buf + count)) {
        if (buffer[0] != '<') {
            next = strnchr(buffer, '<', (buf+count-buffer));
            if (next == NULL) {
                next = buf + count;
            }
        } else {
...
int main(int argc, char **argv)
{
    ...
    struct logger_list *logger_list;
    size_t tail_lines = 0;
	char logvalue[PROPERTY_VALUE_MAX];
    ...
    for (;;) {
        ...
        switch(ret) {
            ...
            case 'n':
				...
				property_get("persist.logcat.num", logvalue, "");
				if((logvalue == NULL)||(strcmp(logvalue,"")==0)||(!isdigit(logvalue[0]))) {
					android::g_maxRotatedLogs = atoi(optarg);
				} else {
					android::g_maxRotatedLogs = atoi(logvalue);
				}
            break;
...
#define  CONSOLE_OPTION "androidboot.console="
    ...
    while (1) {
        ...
        if (android::g_kmsg) {
            ...
            if ((ret = klogctl(KLOG_SIZE_UNREAD, kernel_buffer, sizeof(kernel_buffer)-1)) > 0) {
                if ((ret = klogctl(KLOG_READ, kernel_buffer, sizeof(kernel_buffer)-1)) > 0) {
                    kernel_buffer[ret]='\n';
                    if (android::printKernelBuffer(kernel_buffer, ret) < 0) {
                        perror("write kernel log error");
                    }
        ...
        if (ret < 0) {
            ...
            // MStar Android Patch Begin
            if ((connectRetryCount < 10) && (ret == -ECONNREFUSED)) {
                connectRetryCount++;
                printf("read: Connection refused , retry 10 times(%d).", connectRetryCount);
                usleep(100000);
                continue;
            }
            // MStar Android Patch End
